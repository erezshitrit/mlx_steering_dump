...
RTC_STC_BASE_ID_SEGMENT = "0x0014002C"
ARG_HW_SEGMENT = "Segment Type: 0x1012"
...
class DumpLines:
    """Ordered collection of output lines built up incrementally during WQE dumping."""
    ...
    def add_line(self, line: str):
        """Append a new line to the collection."""
        self.lines.append(line)

    def append_to_line(self, line_index: int, text: str):
        """Append text to the end of an existing line."""
...
class dr_action_verify_counter(dr_action_verify):
    ...
    def verify_action(self, action_info: List[int], dump_lines: DumpLines, line_index: int) -> bool:
        """Verify COUNTER action"""
        action_data = action_info[1]
        stc_info = stc_action_type_info[DR_ACTION_COUNTER]
        stc_allowed_bits = stc_info[2][0]
        # Check if any bits are set outside the 24-bit counter_id field (0x00ffffff)
...
class dr_wqe(ABC):
    ...
    def parse_rtc_dump(self, raw_dump: str) -> int:
        ...
        for line in raw_dump.split('\n'):
            if RTC_STC_BASE_ID_SEGMENT in line:
                i = 2
            elif i > 0:
                if i == 1:
                    stc_base_id = line
...
class dr_wqe_aso(dr_wqe):
    ...
    def parse_wqe_data(self, dump_lines: DumpLines):
        ...
        # Blank line between the WQE CTRL header and the data rows, and
        # tab-indented hex rows, to match the GTA-WQE / GTA-ARG layout
        # (see dr_wqe_gta_arg.parse_wqe_data, dr_wqe_match.print_raw_wqe).
        dump_lines.add_line("")
...
class dr_wqe_gta_arg(dr_wqe):
    ...
    def dr_arg_get_pd(self, arg_index: int) -> int:
        ...
        rdma_device = _config_args.get("dev_name", "")
        ...
        try:
            ...
            # Parse the response to find ARG HW segment
            lines = response.stdout.split('\n')

            for i, line in enumerate(lines):
                if ARG_HW_SEGMENT in line:
    ...
    def parse_wqe_data(self, dump_lines: DumpLines):
        ...
        self.dump_wqe_general_ctrl_seg(dump_lines)
        # Blank line between the WQE CTRL header and the data rows, to
        # match the GTA-WQE spacing (see parse_wqe_gta_data_seg /
        # print_raw_wqe).
        dump_lines.add_line("")
        # Hex rows are tab-indented to match the GTA-WQE "RAW WQE DATA"
        # padding style (see dr_wqe_match.print_raw_wqe).
        for i in range(4, len(self.dwords), 4):
            if i + 4 <= len(self.dwords):
                hex_values = self.get_wqe_line(i, min(i + 4, len(self.dwords)))
                dump_lines.add_line(f"\t{hex_values}")
        ...
        # +2 / +5 (instead of +1 / +4) because the blank spacer above
        # shifted every hex row down by one index in dump_lines.
        for i in range(3):
            dump_lines.append_to_line(i+2, f" ==> GTA ctrl WQE ")
        for i in range(4):
            dump_lines.append_to_line(i+5, f" ==> GTA ARG data ")

class dr_wqe_unknown(dr_wqe):
    """WQE UNKNOWN class for parsing and handling WQE UNKNOWN"""
    def __init__(self, data, gen_ctrl_seg, verbosity=0):
        super().__init__(data, gen_ctrl_seg, verbosity)
        self.type = "DR_WQE_UNKNOWN"
        # Store the raw bytes for parsing
        self.raw_bytes = data
        # Convert bytes to dwords for WQE parsing
        if isinstance(data, bytes):
...
class dr_wqe_error(dr_wqe):
    ...
    Unlike the other dr_wqe_* subclasses, this one is fed a list of CSV-style
    fields (debug header + raw WQE hex words) rather than raw WQE bytes, so
    there is no gen_ctrl_seg from the caller. The trailing fields are joined
    back into a hex string and handed to get_wqe_parser() to build the inner
    WQE parser.
    """

    _KEYS = ["mlx5dr_debug_res_type", "ctx_id", "id",
             "send_engine_index", "syndrom", "raw_data"]
    _HEADER_KEYS = _KEYS[:-1]

    def __init__(self, data, verbosity: int = 4):
        super().__init__(data, None, verbosity)
        self.type = "DR_WQE_ERROR"

        if len(data) > 5:
            raw_data = ' '.join(data[5:])
            processed_data = data[:5] + [raw_data]
        else:
            raw_data = ''
            processed_data = data + ['']
        ...
        self.data = dict(zip(
            self._KEYS,
            processed_data + [None] * (len(self._KEYS) - len(processed_data))
        ))
        self.wqe_parser = get_wqe_parser(raw_data, verbosity)
    ...
    def dump_str(self, verbosity, data=None):
        ...
        out = "WQE Error:\n"
        out += dump_obj_str(self._HEADER_KEYS, self.data)
        out += dump_lines.to_string()
        return out
...
def get_wqe_parser(data: Union[str, List[str]], verbosity: int = 0):
    """Return the appropriate WQE parser instance for the given data.

    Dispatches on the WQE opcode/opmod to one of dr_wqe_match, dr_wqe_gta_arg,
    dr_wqe_aso, or dr_wqe_unknown.

    Args:
        data: Either a hex string or a list whose first element is a hex string
    ...
    if not hex_string:
        raise ValueError("Empty data provided to get_wqe_parser")
    ...
    # Normalize to exactly 128 bytes (32 dwords) by padding or truncating.
...
def dr_wqe_parse_queue(data, num_entries, verbosity=0):
    ...
    The input is expected to be in the textual format produced by the FW
    queue-dump tool, where each WQE is introduced by an index header line of
    the form ``[idx = 0xNNNN]`` followed by lines of hex DWORDs. Header lines
    containing ``"dump of sq"``, ``"connected"``, or rows of ``=`` are
    treated as separators. This parser depends on that exact contract; if the
    upstream format changes, the WQE delimitation logic below needs to be
    updated accordingly.
        ...
        if line.startswith("[idx"):
            ...
            if current_wqe_data and current_idx is not None:
                ...
                if pair_first_idx is None:
                    ...
                    try:
                        wqe_parser = get_wqe_parser(combined_data, verbosity)
                        dump_lines = DumpLines()
    ...
    if current_wqe_data and current_idx is not None and wqe_count < num_entries:
        ...
        if pair_first_idx is None:
            ...
            try:
                wqe_parser = get_wqe_parser(wqe_data_str, verbosity=0)
                dump_lines = DumpLines()
                wqe_parser.parse_wqe_data(dump_lines)
                print(dump_lines.to_string())
            except Exception as e:
            ...
            try:
                wqe_parser = get_wqe_parser(combined_data, verbosity=0)
                dump_lines = DumpLines()
                wqe_parser.parse_wqe_data(dump_lines)
                print(dump_lines.to_string())
            except Exception as e:
        ...
        try:
            wqe_parser = get_wqe_parser(pair_first_data, verbosity=0)
            dump_lines = DumpLines()
            wqe_parser.parse_wqe_data(dump_lines)
            print(dump_lines.to_string())
        except Exception as e:
